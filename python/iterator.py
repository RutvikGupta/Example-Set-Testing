...
class Iterator:
    ...
    def __init__(self, example_list: List[Example], sort_mode: str):
        self.example = example_list
        self.num_examples = len(example_list)
        self.example_sorted = []
        # self.example_sel = []
        self.next_example_sorted = []
    ...
    def first_example(self):
        return self.example[self.example_sorted[0]]

    def last_example(self):
        return self.example[self.example_sorted[-1]]

    def next_example(self):
        if self.curr_ex_index + 1 < self.num_examples:
            return self.example[self.example_sorted[self.curr_ex_index + 1]]
        else:
            return None
